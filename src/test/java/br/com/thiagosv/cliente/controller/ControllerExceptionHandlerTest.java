...
class ControllerExceptionHandlerTest {
    ...
    @Nested
    @DisplayName("Testes para HandleRegistroNaoEncontradoExceptionTests")
    class HandleRegistroNaoEncontradoExceptionTests {

        @Test
        @DisplayName("Deve retornar erro 404 para registro nao encontrado")
        void deveRetornarErro404ParaRegistroNaoEncontrado() {
            ControllerExceptionHandler.ErrorResponse response =
                    handler.handleRegistroNaoEncontradoException(new RegistroNaoEncontradoException(ConstantUtil.ERRO_NOT_FOUND));

            assertThat(response.status()).isEqualTo(HttpStatus.NOT_FOUND.value());
            assertThat(response.mensagem()).isEqualTo(ConstantUtil.ERRO_NOT_FOUND);
        }
    }
    ...
    class ErrorResponseTests {
        ...
        private static Stream<Arguments> gerarErrorResponses() {
            return Stream.of(
                    ...
                    Arguments.of(HttpStatus.UNAUTHORIZED.value(), ConstantUtil.ERRO_NOT_FOUND),
